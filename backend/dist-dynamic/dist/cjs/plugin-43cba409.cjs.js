...
async function createRouter(options) {
  ...
  router.post("/incidents", async (req, response) => {
    ...
    try {
      logger.info("Parsing... ", incident_list);
      logger.info("Parsing... ", incident_list);
      let parsed_incident_list = incident_list;
      logger.info("Successful parse: ", incident_list);
      response.status(200);
      response.json(parsed_incident_list);
    } catch (e) {
      if (typeof e === "string") {
        logger.error("Unsuccessful parse: " + e);
      } else if (e instanceof Error) {
        logger.error("Unsuccessful parse: " + e.message);
      }
      response.status(400);
      response.json({ error: e });
    }
...
//# sourceMappingURL=plugin-43cba409.cjs.js.map
